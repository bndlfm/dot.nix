{ config, pkgs, ... }:
let
  smartYankScript = pkgs.writeShellScript "smartYank.sh" ''
    #!/usr/bin/env bash
    export PATH="${
  ...
  selectWindowAttachTab = pkgs.writeShellScript "attachWindowToCurrentTab.sh" ''
    #!/usr/bin/env bash
    set -euo pipefail
  ...
  selectTabAttachTab = pkgs.writeShellScript "attachTabToCurrentOSWindow.sh" ''
    #!/usr/bin/env bash
    set -euo pipefail
  ...
  programs.kitty = {
    ...
    extraConfig = /* fish */ ''
      ...
      ###############
        ...
        map kitty_mod+y  launch --allow-remote-control --location=hsplit --stdin-add-formatting --stdin-source=@screen_scrollback ${smartYankScript} @active-kitty-window-id
        ...
        map kitty_mod+a>t remote_control_script ${selectTabAttachTab}
        map kitty_mod+a>p remote_control_script ${selectWindowAttachTab}
