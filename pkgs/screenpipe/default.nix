  ...
  backend = rustPlatform.buildRustPackage (
    rec {
      ...
      postPatch = ''
        ...
        # filteredRootLock strips only the 4-line [[patch.unused]] block (keeping the blank line before
        # it), so delete that block by pattern (from its header to EOF) rather than a fixed line count,
        # which silently drifts whenever the upstream Cargo.lock is reformatted.
        sed -i '/half = { git = "https:\/\/github.com\/starkat99\/half-rs.git"/d' Cargo.toml
        sed -i '/^\[\[patch\.unused\]\]/,$d' Cargo.lock
      '';
