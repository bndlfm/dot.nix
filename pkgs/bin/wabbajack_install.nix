...
stdenv.mkDerivation {
  pname = "wabbajack_install";
  version = "0.13";

  src = writeText "wabbajack_install.sh" /*sh*/
    ''
      #!/usr/bin/env bash
  ...
  installPhase =
    ''
      mkdir -p $out/bin
      cp $src $out/bin/waydroid-hide-desktop-entries.sh
      chmod +x $out/bin/waydroid-hide-desktop-entries.sh
    '';
